...
{FILE "{PHP.cfg.themes_dir}/admin/{PHP.cfg.admintheme}/warnings.tpl"}
					<div class="block">
						<h5><i class="fa fa-code"></i> {PHP.L.adm_urls_rules}</h5>
						<div class="wrapper">
<!-- BEGIN: HTA -->
...
<!-- ENDIF -->
						...
						<form name="add_url" id="add_url" action="{ADMIN_URLS_FORM_URL}" method="post" class="ajax">
							<table id="rules" class="table table-bordered">
								<thead>
...
<!-- BEGIN: ROW -->
								<tr id="rule_{ADMIN_URLS_ROW_I}">
									...
									<td class="centerall">
										<a title="{PHP.L.Delete}" href="#" onclick="$(this).parent().parent().remove();return false;" class="btn btn-danger"><i class="fa fa-times"></i> {PHP.L.Delete}</a>
									</td>
								</tr>
<!-- END: ROW -->
								<tr id="rule_new">
...
<!-- ENDIF -->
								<tr>
									<td class="valid" colspan="4">
										<button type="submit" class="btn btn-primary"><i class="fa fa-check-circle"></i> {PHP.L.adm_urls_save}</button>
									</td>
								</tr>
							</table>
						</form>
